...
/*let subCategoria = ['Todas', 'Alimentos', 'Accesorios', 'Juguetes', 'Cuchas y camas'];*/
...
/*function showSubCategorias () {
    ...
    subCategoria.forEach((cat) => {
        ...
        let checked;
        cat == 'Todas' ? checked = 'checked' : '';
        contenedorSubCat.innerHTML = `
        <input class="form-check-input" type="radio" name="subcategoria" id="${cat}" value="${cat}" ${checked}>
...
function seleccionarSubCategoria(e) {
    let filtroCategoria = e.target.value;
    filtroCategoria = filtroCategoria.toLowerCase();
    let productosTienda = document.querySelectorAll('.product');
   ...
   productosTienda.forEach((product) => {
        if(filtroCategoria == 'todas') {
           ...
           if(product.getAttribute('data-subcategoria') == filtroCategoria) {
                product.style.display = 'block';
            } else {
                product.style.display = 'none';
            }
...
function showSelectProduct (e) {
    ...
    let value = $(this).prev('.form-cantidad').find("input").val();
    setTimeout(function() { 
        $('#productoComprado').fadeOut('slow');
    }, 5000);
...
document.querySelectorAll('.increase').forEach(item => {
        item.addEventListener("click", sumarCantidad);
      })

document.querySelectorAll('.decrease').forEach(item => {
        item.addEventListener("click", restarCantidad);
      })

document.querySelectorAll('.form-check-input').forEach(item => {
        item.addEventListener("click", seleccionarSubCategoria);
      })
 }
